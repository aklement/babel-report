...
\setlength{\parskip}{0pt}
  ...
  \item \cite{Mimno:2009} proposed a polylingual topic model and matched high probability words in each topic across languages.  While effective for a small number of topics (448 in their experiments), it is unlikely to scale well for inducing large multilingual dictionaries.  %We propose an alternative to exploit explicit category information already present in many monolingual resources (such as news stories and Wikipedia articles) in \secref{sect:cues}. \mtodo{Keep topic?}
...
%\newpage
\lstinputlisting[caption=Example configuration file for English to Russian bilingual lexicon induction from Wikipedia and newswire data using temporal and contextual cues.  Evaluation is done on 1000 most frequent source words.,label=fig:cfg]{figures/babel.xml}
%\newpage
